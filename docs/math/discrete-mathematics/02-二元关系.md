...
* 如果有序对 $\langle x, y\rangle \in R$，**$xRy$** （读作 $x$ 与 $y$ 有关系 $R$）。
* 如果 $\langle x, y\rangle \notin R$，则记作：**$x \cancel{R} y$**。


* 设 $A, B$ 为集合，$A \times B$ 的任何子集都是从 $A$ 到 $B$ 的关系。
* 当 $A = B$ 时，**$A \times A$ 的任何子集都称作“$A$ 上的二元关系”**。
* **【推论】**：如果集合 $A$ 有 $n$ 个元素（$|A|=n$），那么 $A$ 上能组成多少个不同的二元关系？
  * 推导：$|A \times A| = n^2$ 个有序对。它的任何一个子集都是一个关系，所以子集总数（幂集元素个数）是 **$2^{n^2}$**。
  * *例如*：$A=\{0,1\}$，有 2 个元素。那么 $A$ 上有 $2^{(2^2)} = 2^4 = 16$ 个不同的关系。
